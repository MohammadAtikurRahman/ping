...
// import { Table, Pagination, Container, Form, FormControl, Button } from 'react-bootstrap';
...
//         (transaction.beneficiaryMobile && transaction.beneficiaryMobile.includes(searchTerm)) ||
//         (transaction.beneficiaryId && transaction.beneficiaryId.toString().includes(searchTerm)) ||
//         (transaction.trxid && transaction.trxid.includes(searchTerm)) ||
//         (transaction.timestamp && new Date(transaction.timestamp).toLocaleString().includes(searchTerm))
//       );
//     }

//     if (selectedPhoneNumber) {
//       filtered = filtered.filter(transaction =>
//         transaction.beneficiaryMobile === selectedPhoneNumber
//       );
//     }
...
//   const uniquePhoneNumbers = Array.from(new Set(transactions.map(t => t.beneficiaryMobile))).filter(phoneNumber => phoneNumber);

//   // Filter unique phone numbers based on search term
//   const filteredUniquePhoneNumbers = searchTerm
//     ? uniquePhoneNumbers.filter(phoneNumber => phoneNumber.includes(searchTerm))
//     : uniquePhoneNumbers;
...
//         {selectedPhoneNumber && (
//           <Button variant="secondary" className="ml-2 w-100" onClick={() => setSelectedPhoneNumber('')}>
...
//           </thead>
//           <tbody>
//             {filteredUniquePhoneNumbers.slice(indexOfFirstTransaction, indexOfLastTransaction).map((phoneNumber, index) => (
//               <tr key={phoneNumber}>
//                 <td>{index + 1 + indexOfFirstTransaction}</td>
//                 <td>
//                   <Button variant="link" onClick={() => handlePhoneNumberClick(phoneNumber)}>
//                     {phoneNumber}
//                   </Button>
//                 </td>
